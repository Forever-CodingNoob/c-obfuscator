...
Original Repo: https://github.com/whoward3/C-Code-Obfuscator
"""
...
def variable_renamer(given_string):
    ...
    reserved_keywords = {
        ...
        "transaction_safe", "transaction_safe_dynamic", "import", "module", "co_await", "co_return", "co_yield", "argc", "argv", "NULL"
    }
    index = 0
    new_string = ""
    ...
    filtered_code = re.findall(r"(?:[\w\*\s]*[\w\*]\s[\s\*]*)(?!main)([a-zA-Z_][a-zA-Z0-9_]*)", given_string)
    ...
    for found_example in filtered_code:
        if found_example not in reserved_keywords and found_example not in variable_dictionary:
...
def main():
    ...
    """
    if len(sys.argv)!=2:
        print(f"Usage: {sys.argv[0]} <file_to_obfuscate>")
        exit(1)
